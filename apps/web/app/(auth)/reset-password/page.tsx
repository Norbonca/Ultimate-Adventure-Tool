...
/**
 * Reset Password — design/D01_User_Auth_Profile.pen#G4Huo
 */
...
function getPasswordStrength(password: string): {
  ...
  const weak = { level: 1, label: "weak" as const, barClass: "bg-coral", textClass: "text-coral" };
  ...
  if (types >= 3)
    return {
      ...
      label: "strong",
      barClass: "bg-sage",
      textClass: "text-[color:var(--color-success-text)]",
    };
  if (types >= 2)
    return { level: 2, label: "medium", barClass: "bg-golden", textClass: "text-amber-600" };
  return weak;
}
                    ...
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        ...
                        className={`h-1 flex-1 rounded-full transition-colors ${
                          i <= strength.level ? strength.barClass : "bg-navy-200"
                        }`}
                      />
                    ))}
                  </div>
